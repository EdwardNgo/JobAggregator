...
class Crawler(object):
    ...
    def __init__(self,name,urls,job_list_selector,img_selector,title_selector,company_selector,tags_selector,location_selector,time_selector,main_section_selector,salary_selector,job_details_selector='',job_requirements_selector='',job_other_selector=''):
        self.name = name
        self.urls = urls
        ...
        self.job_details_selector = job_details_selector
        self.job_requirements_selector = job_requirements_selector
        self.job_other_selector = job_other_selector
        ...
        for url in self.urls:
            ...
            while(flag):
                ...
                for item in main_section:
                    ...
                    job['published'] = (datetime.strptime(job['update_time'].replace('/20','/'),"%d/%m/%y") - datetime(1970,1,1)).total_seconds()

                    try:
                        job['month_year'] = getMonthYear(job['update_time'])
                    except:
                        pass
                    res = retryRequest(job['url'])
                    if res.status_code != 200:
                        break
                    soup = BeautifulSoup(res.content,'html.parser')
                    # print(self.job_details_selector)

                    main_content = [removeHtmlTag(str(i)) for i in soup.select(self.job_details_selector)]
                    # requirements = removeHtmlTag('.'.join(soup.select(self.job_requirements_selector)))
                    # other_info = removeHtmlTag('.'.join(soup.select(self.job_other_selector)))
                    job['details'] = main_content
                    # job['requirements'] = requirements
                    # job['other_info'] = other_info
                    # threshold
                ...
                insertdb.insertJobData(jobs,'new_raw_site_job')
